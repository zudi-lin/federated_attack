...
while (True):
    ...
    for curr_id in update_image_data_table:
        ...
        aggregated_img = curr_original_image + GLOBAL_EPS * np.sign(summary)

        # Update summary
        curr_adv_noise = update_image_data_table[curr_id] - curr_original_image
        new_summary = summary + np.sum(np.sign(curr_adv_noise), 0)
